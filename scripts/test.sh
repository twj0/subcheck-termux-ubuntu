...
# SubCheck 测试脚本 - 使用新的项目结构
# 支持优化的网络测试器和配置管理

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_DIR="$(dirname "$SCRIPT_DIR")"
NODES_FILE="$PROJECT_DIR/data/cache/parsed_nodes.json"
RESULTS_FILE="$PROJECT_DIR/data/results/test_results.json"
CONFIG_FILE="$PROJECT_DIR/config/config.yaml"
SUBSCRIPTION_FILE="$PROJECT_DIR/config/subscription.txt"
